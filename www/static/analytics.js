(function () {
  ...
  window.hsSubmitHook = window.hsSumitHook = function (data) {
    ...
    if (fieldMap["email"] && window.rudderanalytics) {
      try {
        window.rudderanalytics.identify({
          ...
          name: [fieldMap["firstname"], fieldMap["lastname"]]
            .filter((a) => !!a)
            .join(" "),
        });
      } catch {}
    ...
    const formId =
      context &&
      context.originalEmbedContext &&
      context.originalEmbedContext.formId
        ? context.originalEmbedContext.formId
        : null;
  ...
  function trackPageLoad(opts) {
    ...
    analytics.page(opts.application, window.document.title, {
