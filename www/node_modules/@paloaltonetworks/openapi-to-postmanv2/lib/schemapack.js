...
class SchemaPack {
  ...
  convert(callback) {
    ...
    generatedStore.collection = new sdk.Collection({
      info: {
        name: _.isEmpty(_.get(openapi, 'info.title'))
          ? COLLECTION_NAME
          : _.get(openapi, 'info.title'),
      },
    });
  ...
  validateTransaction(transactions, callback) {
    ...
    // create and sanitize basic spec
    schema.servers = _.isEmpty(schema.servers)
      ? [{ url: '/' }]
      : schema.servers;
        ...
        (transaction, requestCallback) => {
          ...
          if (typeof requestUrl === 'object') {
            ...
            _.forEach(requestUrl.variable, (pathVar) => {
              if (
                _.isNil(pathVar.value) ||
                (typeof pathVar.value === 'string' &&
                  _.trim(pathVar.value).length === 0)
              ) {
            ...
            return async.map(
              ...
              (matchedPath, pathsCallback) => {
                ...
                _.forEach(matchedPath.pathVariables, (pathVar) => {
                  let mappedPathVar = _.find(
                    _.get(transaction, 'request.url.variable', []),
                    (transactionPathVar) => {
