...
## Expanded Available Large Language Model (LLM) Management APIs

_March 10, 2025_

Vectara expands its Large Language Model (LLM) management capabilities with 
the introduction of the Delete LLM API and Get LLM API, joining the previously 
released Create LLM API. These new LLM endpoints provide organizations with 
greater flexibility in tracking, modifying, and removing external LLM 
configurations, streamlining AI model management while ensuring security and 
compliance.

**Why it matters:** With these enhancements, users can retrieve LLM configurations 
instantly, securely remove outdated models, and maintain an optimized AI 
deployment. The Get LLM API enables users to verify model settings, such as 
authentication and endpoint details, while the Delete LLM API allows 
organizations to clean up unused or deprecated models efficiently.

**New API Endpoints**

* [Get an LLM](/docs/rest-api/large-language-models)
* [Delete an LLM](/docs/rest-api/large-language-models)

**More information:**

* [Get an LLM API Definition](/docs/api-reference/llms-apis/get-llm)
* [Delete LLM API Definition](/docs/api-reference/llms-apis/delete-llm)
