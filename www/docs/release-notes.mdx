...
the retrieved context and the phrasing or intent of the user query. By adding 
the optional query field to VHC requests, you now allow the model to better 
interpret response formatting, resolve ambiguities, and attribute facts that 
are inferred from the query rather than the source alone. This results in 
improved correction accuracy—particularly for instructions like “Answer True or False,” “List the top 3,” or “Summarize concisely.”

**What’s new:**

* The `model` parameter renamed to `model_name`
* Added an optional `query` field for VHC requests
* Query-aware prompt activates when specifying `vhc-large-1.0` as the 
`model_name`.
* Improved correction precision in prompt-sensitive use cases

Usage note: The query parameter is currently supported only with the vhc-large-1.0 model.
...
**New API endpoints:**
* [List Halluncation Correctors](/docs/rest-api/correct-hallucinations)
* [Correct hallucinations in generated text based on source documents](/docs/rest-api/correct-hallucinations)
