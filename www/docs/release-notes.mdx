...
query and chat endpoints. This API enables connectivity with OpenAI-compatible 
models, including Anthropic Claude, Azure OpenAI, and custom-hosted LLMs.

**Why it matters:** Organizations using multiple LLMs now have a streamlined 
way to manage model configurations, authentication, and deployment settings 
through a single API. This update enhances flexibility, allowing users to 
connect to external LLM providers, define custom authentication methods, and 
specify model parameters and API endpoints for their specific use cases.

**New API endpoint:**
* [Create LLM](/docs/api-reference/llms-apis/create-llm)

**More information:**
* Create LLM API Definition
