...
# Querying Data with Vectara

This section explains how to query data using Vectara's API. The code panel on the right shows an example request.
  ...
  snippets={[
    {
      language: 'curl',
      code: `curl -X POST \\
  https://api.vectara.io/v2/corpora/example-corpus/query \\
...
queryVectara();`,
  ...
  annotations={{
    curl: [
      { line: 3, text: 'This header authenticates the request with your API token.' },
      { line: 5, text: 'The JSON body specifies the query and result limit.' },
    ],
    javascript: [
      { line: 4, text: 'Sends a POST request to the Vectara query endpoint.' },
    ],
  }}
/>

Additional content can go here, explaining the response format or other details.