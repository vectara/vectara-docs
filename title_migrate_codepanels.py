...
# Log files
LOG_FILE = 'titled_migration_log.txt'
SUMMARY_FILE = 'titled_migration_summary.txt'
...
def migrate_file(file_path, log_lines, test_mode):
    ...
    # Count and replace fenced code blocks with optional title: ```lang title="Title"\ncode\n```
    fenced_pattern = r'```(\w+)(?:\s+title="([^"]*)")?\s*\n(.*?)\n```'
    fenced_matches = re.findall(fenced_pattern, content, re.DOTALL | re.MULTILINE)
    num_fenced = len(fenced_matches)

    def replace_fenced(match):
        lang = match.group(1)
        title = match.group(2) if match.group(2) else 'Code Example'  # Use captured title or default
    ...
    # (Kept for completeness, though focus is on titled fenced blocks)
    ...
    if total_changes == 0:
        ...
        log_lines.append(f"Would replace {num_fenced} fenced blocks (including titled) and {num_pre} <pre> blocks in {file_path}")
...
# Main execution
parser = argparse.ArgumentParser(description="Migrate titled code blocks to CodePanel")
